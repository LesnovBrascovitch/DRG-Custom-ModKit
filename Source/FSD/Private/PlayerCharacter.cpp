...
AFSDPlayerController* APlayerCharacter::GetPlayerController() const {
    ...
    return Cast<AFSDPlayerController>(GetController());
}
...
UPlayerFPAnimInstance* APlayerCharacter::GetFPAnimInstance() const {
    return Cast<UPlayerFPAnimInstance>(FPMesh->GetAnimInstance());
}
...
void APlayerCharacter::Tick(float DeltaSeconds)
{
    ...
    float TurnRate = GetInputAxisValue(TEXT("Turn"));
    UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - F " + FString::SanitizeFloat(MoveForwardAxis), true, true, FColor::Green, 0.f);
    UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - R " + FString::SanitizeFloat(MoveRightAxis), true, true, FColor::Green, 0.f);
    FVector MovementValue = (GetActorForwardVector() * MoveForwardAxis) + (GetActorRightVector() * MoveRightAxis);
    ...
    if(GetWorldTimerManager().IsTimerActive(timerHandle) == false){
        if (IsValid(PlayerController)) {
            ...
            UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - GetInputAxisValue " + FString::SanitizeFloat(MoveForwardAxis), true, true, FColor::Green, 0.f);

            if (MoveForwardAxis > 0.f) {
                if (GetCharacterMovement()->IsFalling()) {
                    UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - Falling", true, true, FColor::Green, 0.f);
                    FHitResult LedgeFirstHit;
                    FHitResult LedgeSecondHit;
                    ...
                    FVector LedgeFirstStart = GetActorLocation();
                    FVector LedgeFirstEnd = GetActorLocation() + (GetActorForwardVector() * 70);
                    FVector LedgeSecondStart = LedgeFirstEnd;
                    FVector LedgeSecondEnd = (LedgeSecondStart - FVector(0.f, 0.f, 30.f));


                    GetWorld()->LineTraceSingleByChannel(LedgeFirstHit, LedgeFirstStart, LedgeFirstEnd, ECC_GameTraceChannel3, CollisionParameters);
                    //DrawDebugLine(GetWorld(), LedgeFirstStart, LedgeFirstEnd, LedgeFirstHit.bBlockingHit ? FColor::Blue : FColor::Red, false, 0.0f, 0, 1.f);

                    UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - Ledge Ray First " + LedgeFirstHit.bBlockingHit ? "true" : "false", true, true, FColor::Green, 0.f);

                    if (IsValid(LedgeFirstHit.GetActor())) {
                        UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - Ledge Ray First Hit " + LedgeFirstHit.GetActor()->GetName(), true, true, FColor::Green, 0.f);
                    }
                    else {
                        ...
                        if (IsValid(LedgeSecondHit.GetActor())) {
                            ...
                            GetWorld()->GetTimerManager().SetTimer(timerHandle, this, &APlayerCharacter::TimerJump, 1.0f, false);
                            GetWorld()->GetTimerManager().SetTimer(LedgeHiddenItemHandle, this, &APlayerCharacter::LedgeShowItem, 0.625f, false);

                            UAnimMontage* LedgeMontage = LoadObject<UAnimMontage>(nullptr, TEXT("/Game/CharacterStructure/Gear_Unarmed/FP/1P_Unarmed_LedgeGrab_A_Montage.1P_Unarmed_LedgeGrab_A_Montage"));
                            if (LedgeMontage) {
                                FPMesh->GetAnimInstance()->Montage_Play(LedgeMontage);
                            }
                            AItem* EquippedItem = InventoryComponent->GetEquippedItem();
            ...
            {
                UKismetSystemLibrary::PrintString(this, "[C++] PlayerCharacter Tick - NotFalling", true, true, FColor::Green, 0.f);
            }
...
void APlayerCharacter::TimerJump()
{
    GetWorld()->GetTimerManager().ClearTimer(timerHandle);
}


void APlayerCharacter::LedgeShowItem()
{
    AItem* EquippedItem = InventoryComponent->GetEquippedItem();
    ...
    GetWorld()->GetTimerManager().ClearTimer(LedgeHiddenItemHandle);
}
